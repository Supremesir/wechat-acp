 ...
 * weixin-acp ResponseCollector). Returns null if none — caller sends plain text.
 */
export function extractFirstFeedbackMedia(text: string): {
  filePath: string;
  displayText: string;
} | null {
  // 1) [WEIXIN_IMAGE:path] / VIDEO / FILE
  const marker = /\[WEIXIN_(IMAGE|VIDEO|FILE):([^\]]+)\]/;
  let m = text.match(marker);
  if (m) {
    const rawPath = m[2].trim();
    const displayText = text.replace(/\[WEIXIN_(?:IMAGE|VIDEO|FILE):[^\]]+\]\s*/g, "").trim();
