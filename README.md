...
**推荐：[universal-screenshot-mcp](https://github.com/sethbang/mcp-screenshot-server)**（跨平台，支持窗口/全屏/区域截图，Windows 高 DPI 兼容）
...
1. 调用 `take_system_screenshot` 截图：
   - `mode: "window"` + `windowName: "应用名"` — 截取指定窗口
   - `mode: "fullscreen"` — 截取所有显示器（可通过 `display: 1` 指定单个）
   - `mode: "region"` — 截取指定坐标区域
2. 在回复中使用 `[WECHAT_IMAGE:/path/to/screenshot.png]` 引用
3. SDK 自动提取路径并将图片发送到微信

> 也可使用其他截图 MCP，只要能输出本地文件路径即可。
