...
AI Agent 本身无法截图，但可以通过 MCP 工具获取截图能力。在 `~/.cursor/mcp.json` 中添加截图 MCP：
...
```json
{
  "mcpServers": {
    "screenshot-server": {
      "command": "cmd",
      "args": ["/c", "npx", "-y", "universal-screenshot-mcp"]
    }
  }
}
```

> macOS/Linux 下将 `command` 改为 `npx`，`args` 改为 `["-y", "universal-screenshot-mcp"]`。

`weixin-acp` 启动时会自动读取此配置并加载到 ACP session 中。Agent 就可以：

1. 调用 `take_screenshot` MCP 工具截取屏幕/网页
2. 截图保存为本地文件
3. 在回复中使用 `[WEIXIN_IMAGE:/path/to/screenshot.png]` 引用
4. `weixin-acp` 自动提取路径并将图片发送到微信

**排除不需要的 MCP**：如果某些 MCP 在微信 ACP 模式下无意义（如 `relay-mcp`），可在 `AcpAgent` 中配置排除：
