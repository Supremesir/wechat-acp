...
└─────────────────────────────────┬───────────────────────────┘
                                  │ stdio JSON-RPC
┌─────────────────────────────────┴───────────────────────────┐
│  weixin-feedback-server.cjs (MCP Server)                    │
...
| **Relay MCP 自动排除** | ACP 模式下自动排除 `relay-mcp`，避免与 `weixin-feedback` 冲突 |

### 配置

MCP Feedback 需要在 `~/.cursor/mcp.json` 中配置 `weixin-feedback` 服务器（`weixin-acp` 已内置）：
...
```json
{
  "mcpServers": {
    "weixin-feedback": {
      "command": "node",
      "args": ["path/to/weixin-feedback-server.cjs"],
      "env": { "WEIXIN_FEEDBACK_PORT": "19826" },
...
**推荐：[@mcpcn/screenshot-mcp](https://www.npmjs.com/package/@mcpcn/screenshot-mcp)**（轻量，支持多显示器，可保存文件）
...
```json
{
  "mcpServers": {
    "screenshot-server": {
      "command": "npx",
      "args": ["-y", "@mcpcn/screenshot-mcp"],
      "autoApprove": ["capture_screenshot", "list_displays"]
    }
  }
}
```

`weixin-acp` 启动时会自动读取此配置并加载到 ACP session 中。Agent 就可以：

1. 调用 `list_displays` 查看可用显示器
2. 调用 `capture_screenshot` 截取屏幕（支持指定显示器和保存路径）
3. 在回复中使用 `[WEIXIN_IMAGE:/path/to/screenshot.png]` 引用
4. SDK 自动提取路径并将图片发送到微信

> 也可使用其他截图 MCP（如 `universal-screenshot-mcp`），只要能输出本地文件路径即可。

**排除不需要的 MCP**：ACP 模式下 `relay-mcp` 会被自动排除（避免与 `weixin-feedback` 冲突）。如需排除其他 MCP：
